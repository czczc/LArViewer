...
#ifndef DataConverter35t_Module
#define DataConverter35t_Module
...
class DataConverter35t : public art::EDAnalyzer {
public:

    explicit DataConverter35t(fhicl::ParameterSet const& pset);
    virtual ~DataConverter35t();
...
private:
 ...
 }; // class DataConverter35t


//-----------------------------------------------------------------------
DataConverter35t::DataConverter35t(fhicl::ParameterSet const& parameterSet)
...
//-----------------------------------------------------------------------
DataConverter35t::~DataConverter35t()
{
}
...
//-----------------------------------------------------------------------
void DataConverter35t::initOutput()
{
...
//-----------------------------------------------------------------------
void DataConverter35t::beginJob()
{
...
//-----------------------------------------------------------------------
void DataConverter35t::saveChannelWireMap()
{
    ofstream mapfile;
    mapfile.open("ChannelWireMap.txt");
    mapfile << "# total channels: 1992\n\n";
...
//-----------------------------------------------------------------------
void DataConverter35t::saveWireGeometry()
{
    int cstat = 0;
    int tpc = 1;
    int plane = 1;
...
//-----------------------------------------------------------------------
void DataConverter35t::printGeometry()
{
    ...
    for (int i=0; i<fNplanes; i++) {
        cout 
            ...
            << ", wire angle: " << fGeom->WireAngleToVertical(fGeom->Plane(i).View())
...
//-----------------------------------------------------------------------
void DataConverter35t::beginRun(const art::Run& /*run*/)
{
  mf::LogInfo("DataConverter35t")  << "begin run";
}


//-----------------------------------------------------------------------
void DataConverter35t::analyze( const art::Event& event )
{
    reset();
...
//-----------------------------------------------------------------------
void DataConverter35t::processRaw( const art::Event& event )
{
...
DEFINE_ART_MODULE(DataConverter35t)

} // namespace RawDataReader

#endif // DataConverter35t_Module
